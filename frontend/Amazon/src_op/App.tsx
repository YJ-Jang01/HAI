...
export default function App() {
  ...
  const [isAiAgentOpen, setIsAiAgentOpen] = React.useState(false);
  const [aiInitialQuery, setAiInitialQuery] = React.useState<string | undefined>(undefined);

  const handleOpenAiAgent = (withQuery?: string) => {
    setAiInitialQuery(withQuery);
  ...
  return (
    <div className="min-h-screen bg-[#e3e6e6] flex flex-col font-sans text-[#0F1111]" id="applet-viewport">
      ...
      <main className="flex-1 pb-16">
        ...
        {view === 'products' && (
          <ProductListArea
            ...
            onOpenAiAgent={() => handleOpenAiAgent(searchQuery)}
          />
        )}
      ...
      {/* Interactive AI Agent Slider panel */}
      <AiAgentWidget 
        isOpen={isAiAgentOpen}
        onClose={() => setIsAiAgentOpen(false)}
        products={mockProducts}
        onOpenProductDetail={handleOpenDetailModal}
        initialSearchQuery={aiInitialQuery}
      />
