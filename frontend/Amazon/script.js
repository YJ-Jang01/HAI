...
async function init() {
    try {
        const [prodRes, reviewRes] = await Promise.all([
            fetch('products_fixed_sizes_colors.json'),
            fetch('review.json')
        ]);
        ...
        allProducts = allProducts.map(p => {
            ...
            // Origin / Material 필드 통합
            copy.origin = copy.origin || copy.Origin;
            copy.material = copy.material || copy.Material;
            return copy;
        });
...
function showCategory(category) {
    ...
    const origins = [...new Set(categoryProducts.map(p => p.Origin).filter(Boolean))];
    const materials = [...new Set(categoryProducts.map(p => p.Material).filter(Boolean))];
    const sizes = [...new Set(categoryProducts.flatMap(p => p.sizes || []).filter(Boolean))];
    ...
    categoryProducts.forEach(prod => {
        const szs = prod.sizes || [];
        szs.forEach(s => {
            if (!s) return;
            const groupLabel = s.trim().toUpperCase();
            if (!sizeGroupmap.has(groupLabel)) {
                sizeGroupmap.set(groupLabel, groupLabel);
            }
        });
    });
    const sizeArray = Array.from(sizeGroupmap.values());
    const sizeFilterHTML = `
                <h3>Size</h3>
                <ul class="filter-list filter-list-vertical">
                    <li class="size-dot" onclick="updateFilter(this, 'size', 'All')" style="background: transparent; border: 1px solid #ccc; display: flex; align-items: center; justify-content: center; font-size: 10px; color: #555;">All</li>
                    ${sizeArray.map(s => `<li class="size-dot" onclick="updateFilter(this, 'size', '${s}')">${s}</li>`).join('')}
                </ul>
    `;
    ...
    productView.innerHTML = `
        <div class="category-page-layout" style="display: flex; gap: 20px; padding: 20px;">
            <aside class="sidebar-filter" style="width: 200px; flex-shrink: 0;">
                ...
                <h3>Material</h3>
                <ul class="filter-list filter-list-vertical">
                    <li class="filter-item active" onclick="updateFilter(this, 'material', 'All')">All</li>
                    ${materials.map(m => `<li class="filter-item" onclick="updateFilter(this, 'material', '${m}')">${m}</li>`).join('')}
                </ul>
                ` : ''}
                ...
                <h3>Origin</h3>
                <ul class="filter-list filter-list-vertical">
                    <li class="filter-item active" onclick="updateFilter(this, 'origin', 'All')">All</li>
                    ${origins.map(o => `<li class="filter-item" onclick="updateFilter(this, 'origin', '${o}')">${getOriginIcon(o)} ${o}</li>`).join('')}
                </ul>
                ` : ''}
            </aside>
...
function applyFilters() {
    ...
    const filtered = allProducts.filter(p => {
        const matchCategory = !currentFilters.mainCategory || p.category === currentFilters.mainCategory;
        ...
        const matchOrigin = currentFilters.origin === 'All' || p.origin === currentFilters.origin;
        const matchMaterial = currentFilters.material === 'All' || p.material === currentFilters.material;

        return matchCategory && matchSub && matchPrice && matchRating && matchColor && matchOrigin && matchMaterial;
...
function renderSearchResults(products, title, selectedCategory = 'All') {
    const productView = document.getElementById('product-view');
    const homeView = document.getElementById('home-view');
    const sideCartView = document.getElementById('side-cart-view');
    ...
    if (sideCartView) {
        if (cart && cart.length > 0) sideCartView.classList.remove('hidden');
        else sideCartView.classList.add('hidden');
    }
    ...
    currentFilters.mainCategory = selectedCategory === 'All' ? '' : selectedCategory;
    currentFilters.subCategory = 'All';

    const subCategories = [...new Set(filterProducts.map(p => p.subCategory).filter(Boolean))];
    const maxProductPrice = filterProducts.length > 0
        ? Math.max(...filterProducts.map(p => {
            const price = typeof p.price === 'string' ? parseFloat(p.price.replace(/[^0-9.]/g, '')) : p.price;
            return price || 500;
        }))
        : 500;
    const filterMaxPrice = currentFilters.maxPrice === Infinity ? maxProductPrice : currentFilters.maxPrice;
    const origins = [...new Set(filterProducts.map(p => p.Origin || p.origin).filter(Boolean))];
    const materials = [...new Set(filterProducts.map(p => p.Material || p.material).filter(Boolean))];
    ...
    productView.innerHTML = `
        <div class="category-page-layout" style="display: flex; gap: 20px; padding: 20px;">
            <aside class="sidebar-filter" style="width: 220px; flex-shrink: 0;">
                <h3>Department</h3>
                <ul class="filter-list filter-list-vertical">
                    <li onclick="updateFilter(this, 'subCategory', 'All')">All</li>
                    ${subCategories.map(sub => `<li onclick="updateFilter(this, 'subCategory', '${sub}')">${sub}</li>`).join('')}
                </ul>
                <h3>Price</h3>
                <div style="display: flex; flex-direction: column; gap: 10px;">
                    <input type="range" min="0" max="${maxProductPrice}" value="${filterMaxPrice}" oninput="handlePriceSlider(this.value)" style="width: 100%;">
                    <span id="max-price-label">Up to $${filterMaxPrice}</span>
                </div>
                <h3>Customer Review</h3>
                <ul class="filter-list">
                    <li onclick="updateFilter(this, 'minRating', 4)"><span class="stars-gold">★★★★☆</span> & Up</li>
            ...
            <div class="product-list-area" style="flex-grow: 1;">
                <div class="results-header" style="margin-bottom: 20px;">
                    <h2>${title}</h2>
                    <p id="current-category-count">${products.length} items found</p>
                </div>
                <div id="product-list" class="product-grid" style="display: grid; grid-template-columns: repeat(auto-fill, minmax(220px, 1fr)); gap: 20px;">
            ...
            <aside id="side-cart-container" style="width: 300px; flex-shrink: 0; ${cart && cart.length > 0 ? '' : 'display:none;'}">
                <div style="background: white; padding: 20px; border-radius: 8px; border: 1px solid #ddd; position: sticky; top: 20px;">
                    ...
                    <p style="font-weight: bold; margin-top: 15px; font-size: 14px;">소계: ₩ ${cart && cart.length > 0 ? cart.reduce((sum, item) => {
...
function handleSearch() {
    ...
    if (query) {
        filtered = filtered.filter(p => {
            const searchTarget = [
                ...
                p.Material,
                ...(p.features || [])
            ].filter(Boolean).join(' ').toLowerCase(); // 존재하는 값들만 모아서 하나의 문자열로 결합
            
            return searchTarget.includes(query);
        });
        
        const categoryTitle = document.getElementById('category-title');
        if (categoryTitle) categoryTitle.innerText = `Search results for "${query}"`;
    } else {
        const categoryTitle = document.getElementById('category-title');
        if (categoryTitle) categoryTitle.innerText = selectedCategory === "All" ? "All Products" : `Results for "${selectedCategory}"`;
    }
    ...
    renderSearchResults(filtered, title, selectedCategory);
}
